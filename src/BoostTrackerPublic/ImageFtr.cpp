...
Matrixu			HaarFtr::toViz()
{
	...
	{
		if( _weights[k] < 0 )
			v.drawRect(_rects[k],1,(int)(255*max((double)-1*_weights[k],0.5)),0,0);
		else
			v.drawRect(_rects[k],1,0,(int)(255*max((double)_weights[k],0.5)),(int)(255*max((double)_weights[k],0.5)));
	}
