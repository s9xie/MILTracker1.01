...
template<> void					Matrixu::display(int fignum, float p)
{
	...
	cvResizeWindow( name, max((int)(_cols*p),200), (int)max((int)(_rows*p),(int)(_rows*(200.0f/_cols))) );
	//cvWaitKey(0);//DEBUG
	freeIpl();
}
...
template<> Matrixu				Matrixu::imResize(float r, float c)
{
	...
	for (int k = 0; k < _depth; k++) {
		...
		ippiResizeGetSize_8u(_roi, res._roi, ippLinear, 0, &specSize, &initSize);
		Ipp8u *pInitBuf = ippsMalloc_8u(initSize);
		IppiResizeSpec_32f* pSpec = (IppiResizeSpec_32f*)ippsMalloc_8u(specSize);

		ippiResizeGetBufferSize_8u(pSpec, res._roi, numLobes, &bufSize);
		Ipp8u* pBuffer = ippsMalloc_8u(bufSize);
		ippst = ippiResizeLinear_8u_C1R((Ipp8u*)_data[k], _dataStep, (Ipp8u*)res._data[k], res._dataStep,
			dstOffset, res._roi,
			ippBorderRepl, 0,
			pSpec, pBuffer);
